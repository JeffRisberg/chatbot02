...
function TaskList(props) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (scope === 'weekly') {
        const result = await axios("localhost:5000/api/weekly_tasks/" + user_id + "?done=" + done);
        setData(result.data.slice(0, 7));
      }
    })();
  }, [props]);
  ...
  function submit(task_id) {
    if (checkedTaskIds.includes(task_id)) {
      ...
      var url = "localhost:5000/api/mark_daily_task_done"
      if (scope === 'weekly') {
          url = "localhost:5000/api/mark_weekly_task_done"
      }
